...
./server --posts
```
*Note: this is only needed to get a list of dynamic URLs generated from `content.json` or other `.json` files needed for the `wget` download to produce static output. If a URL is static (i.e. not an entry in a list somewhere in a `json` file), simply hardcode it wherever it is needed (e.g. `/posts/feed.xml`).*
